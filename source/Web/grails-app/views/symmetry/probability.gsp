<html>
<head>
    <theme:title><g:message code="symmetry.confprobability.title" default="Shift-Symmetric Configuration Probability" /></theme:title>
    <nav:set path="app/symmetry/Probability"/>
    ...
    <r:script>
        function showCallback(data) {
            ...
            if (data.errors) {
                ...
                $("#outputLabel").html("<p>The probability of selecting a shift-symmetric configuration for the lattice " + latticeSize + "x" + latticeSize + " with the alphabet size (states per cell) " + alphabetSize + " is:</p>");
...
<theme:zone name="body">
    <ui:form remote="true" name="probabilityForm" url="[controller: 'symmetry', action: 'calcTwoDimProbability']" before="hideOutput();" onSuccess="showCallback(data);">
        ...
        <ui:field label="Distribution Type">
            <ui:fieldInput>
                <g:select id="distributionType" name="distributionType" from="${["Uniform", "Density-Uniform"]}"/>
            </ui:fieldInput>
        </ui:field>
