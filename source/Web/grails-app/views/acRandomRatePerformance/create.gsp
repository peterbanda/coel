...
<html>
<head>
	<theme:title>Run Random Rate Performance Evaluation</theme:title>
...
<body>
	<theme:zone name="body">
		<div id="performanceEvaluationLaunchSuccess-Dialog" title="Performance Evaluation">
			<p>Random rate performance evaluation has been successfully launched.</p>
		</div>
		<div> 
			<ui:form remote="true" name="runPerformanceEvaluationForm" url="[controller: 'acRandomRatePerformance', action: 'runPerformanceEvaluation']" before="showPerformanceEvaluationLaunchSuccessDialog();" >
				...
				<ui:field bean="instance" name="randomRateGenerationNum"/>

				<ui:field label="Rate Constant Type Bounds">
					<ui:fieldInput>
						<g:select name="rateConstantTypeBoundIds" from="${AcRateConstantTypeBound.list(sort: 'id')}"
							multiple="yes"
							optionKey="id"
							optionValue="${{it.id + ' : ' + it.rateConstantType.toString() + '  (' + it.bound?.from + ' - ' + it.bound?.to + ')'}}"
							size="8"/>
					</ui:fieldInput>
				</ui:field>
				</hr>
